...
var world2_info = [
    """
    Welcome to World 2, the Internet Maze!
    In this puzzle you will be exploring web vulnerabilities. The internet is where many cybersecurity attacks occur and knowing how these attacks work is already half the battle.
    [i]password = pet_name,2,3[/i]
    """,
    """
    This is a Man In The Middle Attack, its a technique where an outside actor acts as a medium between two people communicating.
    Sometimes its used for snooping in on conversations and other times like youll see in this level, the actor can try to phish the users for information
    Phishing a common social engineering tactic to gain information from unsuspecting users.
    """,
    """
    A SQL injection occurs when code that executes SQL queries is triggered from input.
    An example SQL query is SELECT * FROM Table WHERE name == ' + x + '\''. 
    x normally is user data like name = bob which executes => SELECT * FROM Table WHERE name == 'bob'
    However, if x is a special character ' this can break out of the SQL statement and the hacker can put whatever following the '
    [i]What are the initals of Cross Site Scripting?[/i]
    """,
    """
    Cross-site scripting or XSS, is a type of cyber attack that allows for external code to be run on the targeted server normally through a website.
    This can happen if as a developer you do not properly sanitize the files when uploading or input. Attackers normally use a server and POST their code as
    the main way of execution
    [i]What is the special character used for SQL injections?[/i]
    """,
...
func get_book_info() -> String:
    match (current_world):
        ...
        "world2":
            return world2_info[book_number]
        "world3":
